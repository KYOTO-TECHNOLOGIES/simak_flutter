...
class _AddAddressDialogState extends State<AddAddressDialog> {
  ...
  bool get _isPhoneValid {
    final country = _countries.firstWhere((c) => c['code'] == _selectedCountryCode);
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildDropdown(
                          ...
                          itemLabelBuilder: (v) => tr(context, 'address_type_${v.toLowerCase()}'),
                      ...
                      children: [
                        ...
                        _buildTextField(
                          ...
                          validator: (v) {
                            ...
                            if (!_isNameValid) return tr(context, 'address_name_min_length');
                            return null;
                          },
                        ),
                        const SizedBox(height: 16),
                        ...
                        _buildPhoneField(theme, isLocked, isPhoneVerified, user),
                        ...
                        Row(
                          children: [
                            ...
                            Expanded(child: Text(tr(context, 'address_set_default'), style: const TextStyle(fontSize: 12))),
                          ],
                        ),
                      ],
                    ),
                  ...
                  Expanded(
                    ...
                    child: SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: Text(tr(context, 'address_save_btn'), style: const TextStyle(fontWeight: FontWeight.bold)),
                      ),
                    ),
                  ),
  ...
  Widget _buildPhoneField(ThemeData theme, bool isLocked, bool isPhoneVerified, UserModel? user) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildTextField(
          controller: _phoneController,
          hintText: tr(context, _countries.firstWhere((c) => c['code'] == _selectedCountryCode)['hint']!),
          keyboardType: TextInputType.phone,
          readOnly: isLocked,
          maxLength: _countries.firstWhere((c) => c['code'] == _selectedCountryCode)['maxLength'] as int,
          inputFormatters: [
            FilteringTextInputFormatter.digitsOnly,
            LengthLimitingTextInputFormatter(
              _countries.firstWhere((c) => c['code'] == _selectedCountryCode)['maxLength'] as int,
            ),
          ],
          validator: (v) {
            if (v!.isEmpty) return tr(context, 'required');
              ...
              onSelected: (country) {
                setState(() {
                  ...
                  if (_phoneController.text.length > newMaxLength) {
                    _phoneController.text = _phoneController.text.substring(0, newMaxLength);
                  }
                });
              },
              itemBuilder: (context) => _countries.map((c) => PopupMenuItem<Map<String, dynamic>>(
                value: c,
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    ...
                    Text(c['code']!, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                  ],
                ),
              )).toList(),
              child: Padding(
          ...
          InkWell(
              ...
              if (user != null) {
                ...
                showDialog(
                  ...
                  builder: (context) => OtpVerificationDialog(
                    type: 'phone',
                    currentValue: phone.length > _selectedCountryCode.length ? phone : (user.phoneNumber ?? ''),
                    userId: user.id,
            ...
            child: Padding(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.verified_user_outlined, size: 14, color: AppColors.actionBlue),
                  const SizedBox(width: 4),
                  Flexible(
                     child: Text(
  ...
  void _submit() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      String fName = nameParts.length > 1 ? nameParts.sublist(0, nameParts.length - 1).join(' ') : fullName;
      String lName = nameParts.length > 1 ? nameParts.last : '.'; // Better fallback for backend validation

      // Construction: Flat/Villa, Building, Street Address 
      final line1 = '${_flatVillaController.text}, ${_buildingController.text}, ${_streetController.text}';
      
      final address = AddressModel(
        type: _type,
        firstName: fName,
        lastName: lName,
        phoneNumber: '$_selectedCountryCode${_phoneController.text.replaceAll(RegExp(r'[^0-9]'), '')}', // Prepended country code
        line1: line1,
      ...
      if (success) {
        if (addressController.selectedAddress != null) {
          checkoutController.selectAddress(addressController.selectedAddress!.id!);
        }
        if (mounted) Navigator.pop(context);
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(tr(context, 'failed_to_add_address')), backgroundColor: AppColors.error),
          );
        }
      }
    }
